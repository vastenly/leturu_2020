...
public class CucumberReportUtils {
	...
	public synchronized static void parseScenario(Scenario scenario) {
		try {
			Field junitReporterField = scenario.getClass().getDeclaredField("reporter");
			junitReporterField.setAccessible(true);
	        JUnitReporter reporter = (JUnitReporter) junitReporterField.get(scenario);
